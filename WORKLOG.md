...
- 전체적인 방향은 올바르게 진행되고 있습니다.
- 메인 포트폴리오, EXPERIENCE 캐러셀, 경험별 상세 페이지, Q&A, 로그인·회원가입 페이지가 구분되어 있습니다.
- 데스크톱과 모바일 화면을 함께 고려하고 있으며 빌드와 GitHub 동기화가 안정적으로 유지되고 있습니다.
- 변경 사항을 작은 커밋으로 나누고 작업 일지를 누적하는 진행 방식은 계속 유지합니다.

#### 우선 개선사항

1. 로그인 기능의 목적 결정
   - 로그인한 사용자가 할 수 있는 기능이 아직 없습니다.
   - Q&A 질문 작성, 방명록, 프로필 관리 등 회원가입이 필요한 이유를 먼저 정해야 합니다.
2. Supabase 실제 연결
   - 화면과 인증 코드는 준비됐지만 환경 변수가 없어 실제 계정 생성, 로그인, 세션 유지가 작동하지 않습니다.
3. 개인정보 수집 기준 마련
   - 출생년도, 성별, 지역의 수집 목적과 보관 기간, 수정·삭제 방법을 마련해야 합니다.
   - 반드시 필요하지 않은 정보는 선택 입력으로 전환하는 방안도 검토합니다.
4. 연락 정보 정리
   - 푸터의 `hello@example.com`을 실제 연락처로 교체하거나 준비 전까지 제거해야 합니다.

#### 코드와 운영 방식 개선사항

- 한 줄 중심으로 압축된 CSS를 base, navigation, carousel, detail, auth, responsive 영역으로 나눠 관리합니다.
- 상세 HTML 페이지의 들여쓰기와 구조를 정리해 수정하기 쉽게 만듭니다.
- 여러 페이지에서 반복되는 헤더, 푸터, 폰트 설정을 공통화하는 방안을 검토합니다.
- HTML 유효성, 접근성, 내부 링크, 빌드 검사를 자동화합니다.
- 새 페이지는 빌드뿐 아니라 실제 배포 주소에서 직접 접근되는지도 확인합니다.

#### 권장 진행 순서

1. 사이트 방문 목적과 로그인 사용자 기능 결정
2. 현재 화면 구조 확정
3. CSS와 HTML 구조 정리
4. Supabase 연결 및 실제 인증 테스트
5. 개인정보 처리와 프로필 관리 구현
6. 자동 검사와 배포 검증 추가
7. 프로젝트 이미지와 실제 결과물 보강

#### 다음 결정사항

- 로그인한 사용자가 이 사이트에서 무엇을 할 수 있어야 하는지 결정합니다.
